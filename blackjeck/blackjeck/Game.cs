    ...
    internal class Game : Coin
    {
        ...
        public void D_open(ref Card card)
        {
            ...
            if (!p_bj)
            {
                ...
                {
                    for (int i = 0; i <= d_hit_count + 1; i++)
                    {
                        if (card.SwapA(ref card.d_number[i], card.d_s_number[i]) == true)             //A를 포함하는지 검사
                        {
                            if (flag == 0)                                                           // A가 여러개일때를 가정
                            {                                                                       // flag가 0일때 (A가 처음일때)
                                d_total -= 10;
                                flag++;
                                Console.WriteLine("A(에이스)를 1로 변환 중..."); Thread.Sleep(1500); Console.WriteLine("합: " + d_total); Thread.Sleep(1500);
                                continue;
                            }
                            else
                            {                                                                  // 이미 한번 -10했으므로 컨티뉴
                    ...
                    else if (d_total >= 17 && d_total <= 20) { Thread.Sleep(1000); Compare(d_total, p_total, win); }
                    else if (d_total == 21) { Thread.Sleep(1000); Console.WriteLine("\n딜러 블랙잭!!\n"); Thread.Sleep(1000); Compare(d_total, p_total, win); }
                    else if (d_total >= 22) { win = false; Thread.Sleep(1000); Compare(d_total, p_total, win); }
                }
            }
            else
            {
        ...
        {
            Console.WriteLine("딜러의 카드를 오픈하겠습니다.\n");
            Thread.Sleep(1500);
            Console.WriteLine("딜러의 카드는 " + card.d_patten[0] + " " + card.d_s_number[0] + ", " + card.d_patten[1] + " " + card.d_s_number[1]);
            Console.WriteLine("합: " + d_total);
            Console.WriteLine();
        ...
        public void Play()
        {
            ...
            if (Bj_Check(p_total) == true)                                          // 플레이어 블랙잭 여부
            {
                p_bj = true;
                P_blacksay();
            ...
            switch (ans)
            {
                ...
                case "H":
                    ...
                    if (p_total < 21) { P_HitSay(ref card); goto Re; }                                    // 21보다 작을때 카드출력후 히트여부
                    else if (p_total == 21) { P_HitSay(ref card); P_blacksay(); Dealer_hit(ref card); Player_stay(ref card); }   // 블랙잭 일때 
                    else {                                                                              // 21이상일 때
                        for(int i= 0; i <= p_hit_count+1; i++)
                        {
                        ...
                        else if (p_total == 21) { P_HitSay(ref card); P_blacksay(); Dealer_hit(ref card);} // A를변환했을때 블랙잭이면
